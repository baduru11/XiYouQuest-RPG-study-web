  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt === retries) {
        if (fallbackFn) {
          console.log(
            `[AI] Primary model exhausted ${retries + 1} attempts, trying fallback...`,
          );
      ...
      console.warn(
        `[AI] Attempt ${attempt + 1} failed, retrying in ${Math.round(delay)}ms...`,
      );
...
async function chatCompletion(
  ...
  console.log(
    `[AI] chatCompletion (${model}) responded in ${Date.now() - start}ms, status ${res.status}`,
  );
...
async function fetchCompletion(
  ...
  console.log(
    `[AI] quickCompletion (${model}) ok in ${Date.now() - start}ms, length=${content.length}`,
  );
  if (!content)
    console.warn(
      `[AI] quickCompletion empty response:`,
    ...
    try {
      ...
      const cause =
        err instanceof Error && "cause" in err
          ...
          : "";
      console.warn(
        `[AI] quickCompletion attempt ${attempt + 1} failed: ${err instanceof Error ? err.message : err}${cause}`,
      );
    }
  }
  try {
    ...
    const cause =
      err instanceof Error && "cause" in err
        ...
        : "";
    console.warn(
      `[AI] quickCompletion fallback failed: ${err instanceof Error ? err.message : err}${cause}`,
    );
  }
...
function parseC5Analysis(text: string): GeminiC5Analysis {
  ...
  return {
    ...
    vocabularyNotes: String(
      parsed.vocabularyNotes || "Standard vocabulary usage.",
    ),
    fluencyLevel: fluencyLevel,
    fluencyNotes: String(parsed.fluencyNotes || "Generally fluent delivery."),
    contentRelevance: String(
      parsed.contentRelevance || "Content is relevant to the topic.",
    ),
  };
}
...
Analyze this speaking sample according to the PSC C5 rubric.`;
  ...
  try {
    ...
    return {
      vocabularyLevel: 2,
      vocabularyNotes:
        "Unable to analyze vocabulary — defaulting to Level 2.",
      fluencyLevel: 2,
...
export function buildPhasePrompt(input: PhaseGenerationInput): string {
  const totalPhases = input.totalCheckpoints + 1;
  const totalNodes = Math.min(
    60,
    Math.max(18, Math.round(input.daysRemaining * 2)),
  );
  const nodesPerPhase = Math.min(
    20,
    Math.max(6, Math.round(totalNodes / totalPhases)),
  );
  ...
  const availableLines = Object.entries(input.availableQuestionCounts)
    .map(
      ([comp, count]) =>
        `  - Component ${comp} (${COMPONENT_LABELS[Number(comp)] ?? `C${comp}`}): ${count} questions available`,
    )
    .join("\n");

  const historySection =
    input.previousPhases && input.previousPhases.length > 0
      ? `\nPrevious phase history:\n${input.previousPhases
          .map((pp) => {
            const ppScores = Object.entries(pp.scores)
              .map(([k, v]) => `${k}: ${v}`)
              .join(", ");
            const ppNodes = pp.completedNodes
...
function parsePhaseOutput(text: string): PhaseGenerationOutput {
  ...
  const nodes: CurriculumNode[] = rawNodes.map(
    (n: Record<string, unknown>) => ({
      ...
      estimatedMinutes: estimateMinutes(
        Math.min(7, Math.max(1, Number(n.component) || 1)),
        Math.min(15, Math.max(1, Number(n.questionCount) || 8)),
      ),
    }),
  );
...
export async function generatePhase(
  ...
  const userPrompt =
    "Generate the analysis and curriculum nodes for this phase.";

  console.log(
    `[AI] Phase ${input.phaseNumber} prompt size: ${systemPrompt.length} chars`,
  );
  try {
    const text = await retryWithBackoff(
    ...
    console.log(
      `[AI] Phase ${input.phaseNumber} response size: ${text.length} chars`,
    );
    return parsePhaseOutput(text);
  } catch (error) {
    console.error(
      "[AI] Phase generation failed:",
      error instanceof Error ? error.message : error,
    );
    throw new Error(
      "AI curriculum generation temporarily unavailable. Please try again.",
    );
  }
}
