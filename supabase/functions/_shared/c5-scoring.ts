import type { PronunciationAssessmentResult } from "./iflytek-ise.ts";
import type { GeminiC5Analysis } from "./ai-client.ts";
...
function scorePronunciation(iseResult: PronunciationAssessmentResult): {
  score: number;
  deduction: number;
  level: number;
  label: string;
  ...
  const errorCount = iseResult.words.filter(
    (w) => w.errorType !== "None",
  ).length;
  ...
  const deduction =
    Math.round((maxDeduct - (maxDeduct - minDeduct) * t) * 10) / 10;
...
function scoreVocabGrammar(geminiAnalysis: GeminiC5Analysis): {
  score: number;
  deduction: number;
  level: number;
  label: string;
...
export function calculateC5Score(params: {
  ...
  const { iseResult, geminiAnalysis, spokenDurationSeconds, transcript } =
    params;
  ...
  const totalScore =
    Math.round(Math.max(0, Math.min(30, rawTotal)) * 10) / 10;
