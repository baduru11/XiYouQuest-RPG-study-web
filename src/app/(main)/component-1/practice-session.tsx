...
export function PracticeSession({ questions, character, characterId, component, lpNodeId }: PracticeSessionProps) {
  ...
  const handleRecordingComplete = useCallback(async (audioBlob: Blob) => {
    ...
    setDialogue(`Let me check all ${currentWords.length} characters...`);
    ...
    try {
      ...
      formData.append("category", "read_syllable");
      ...
      if (!assessResponse.ok) {
        ...
        console.error("[C1] Assessment API error:", assessResponse.status, errBody);
      ...
      // iFlytek ISE returns words in order matching the input for read_syllable.
          ...
          {phase === "feedback" && wordScores.length > 0 && (
            <div className="pixel-border bg-card p-4 space-y-3">
              ...
              <div className="border-t pt-3">
                ...
                <div className="space-y-1">
                  <p className="text-lg"><span className="font-bold text-green-600">100</span> Correct</p>
                  <p className="text-lg"><span className="font-bold text-yellow-600">40</span> Wrong tone</p>
                  <p className="text-lg"><span className="font-bold text-red-600">20</span> Tone + vowel</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 md:w-[70%]">
          <Card className="h-full">
            <CardContent className="flex flex-col items-center justify-center gap-6 py-8">
              <p className="self-start text-lg sm:text-2xl text-muted-foreground">🔊 Tap any character to listen</p>
              <div className="flex items-center gap-2">
                <Button
              ...
              {/* 10-word grid display (2 rows of 5) */}
                ...
                {currentWords.map((word, idx) => (
                  <div key={idx} className="space-y-1">
                    <button
                      ...
                      className={`w-full flex items-center justify-center rounded-lg border-2 p-4 transition-colors cursor-pointer
